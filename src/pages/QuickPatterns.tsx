...
const matchesPattern = (number: NumberData, pattern: string): boolean => {
  ...
  switch(pattern) {
    case "Without 2 4 8, Not More than 2 times":
      return /^(?!.*[248])\d{10}$/.test(digits) && !Object.values([...digits].reduce((acc, d) => (acc[d] = (acc[d] || 0) + 1, acc), {})).some(v => v > 2);;
    case "3 DIGIT":
    ...
    setTimeout(() => {
      try {
        ...
        customPatterns.forEach(pattern => {
          formattedCategories[pattern] = uploadedNumbers
            .filter(number => matchesPattern({ number: number.toString() }, pattern))
            .map((num, index) => ({
          ...
          formattedCategories[pattern].forEach(num => {
            let sum = num.digitSum;
            while (sum >= 10) {
              sum = sum.toString().split('').reduce((s, d) => s + parseInt(d), 0);
            }
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (selectedPattern === 'SuperVIP') {
        ...
        setDisplayedNumbers(categorizedNumbers[selectedPattern] || []);
      } else {
        setDisplayedNumbers([]);
      }
